...
function startApp(settings) {
  ...
  function genericRESTHandler(req, res) {
    ...
    proxyRequest(url, method, headers, body)
  ...
  function proxyRequest(uri, method, headers, body) {
    return new Promise((resolve, reject) => {
      ...
      const req = http.request(
        { host: serverHost, port: serverPort, path: uri, method, headers },
        (res) => {
          const chunks = [];
  ...
  function startWSServer(app, server) {
    ...
    wsServer.on("request", function (request) {
      if (
        !approved_origins ||
        (isArray(approved_origins) &&
          approved_origins.indexOf(request.origin) >= 0)
      ) {
        ...
        if (connected_ws_clients.length > settings.websocket.maxClients) {
          ...
          if (settings.logging.websocket) {
            ...
            appLog(
              "error",
              "an attempt to connect to the websocket server was rejected because the maximum number of clients was exceeded."
            );
          }
          connection.close();
          return;
  ...
  function appLog() {
    ...
    if (settings.logging && settings.logging.online) {
      ...
      if (
        settings.logging.online.ws &&
        connected_ws_clients &&
        connected_ws_clients.length > 0
      ) {
...
function getRootHtml() {
  if (!getRootHtml.cache) {
    ...
    const styles_css = fs.existsSync(styles_css_path)
      ? fs.readFileSync(styles_css_path)
      : "";
  ...
  ws.onmessage = function (evt) {
    ...
    try {
      ...
      if (!!payload && typeof payload.console === "object") {
        ...
        return console.error("invalid console message: " + msg);
      }
...
function getFunctionSource(fn) {
  ...
  const fnSource = fnString.substring(
    fnString.indexOf("{") + 1,
  ...
  if (settings.translate && settings.translate.GET) {
    for (let uri in settings.translate.GET) {
      ...
      if (
        ...
        operation.POST.urlParseContent
      ) {
